...
func TestPRK_Generate_GeneratesCorrectShortKey(t *testing.T) {
	...
	key1, err := hk.Generate(10, []byte("some key 1"))
	if err != nil {
	...
	key2, err := hk.Generate(10, []byte("some key 2"))
	if err != nil {
		t.Fatalf("unexpected error: %s", err)
	}
...
func TestPRK_Generate_GeneratesCorrectLongKey(t *testing.T) {
	secret := []byte{0xfa, 0x1d, 0xa5, 0x8e, 0xaa, 0x63, 0xb0, 0x8b}
	salt := []byte{0x30, 0x46, 0x48, 0xa7, 0xb4, 0x5a, 0x61, 0xa4}
	expected := []byte{
	...
	key, err := hk.Generate(80, []byte("some long key"))
	if err != nil {
		t.Fatalf("unexpected error: %s", err)
	}
...
func TestPRK_Generate_GeneratesCorrectKeyForEmptyInfo(t *testing.T) {
...
func TestPRK_Generate_FailsForTooLongKeyRequest(t *testing.T) {
	hk := New(sha256.New, nil, nil)

	// The maximum is 255 * hash-size = 8160-octet long.
	if _, err := hk.Generate(8160, nil); err != nil {
		t.Errorf("unexpected error: %s", err)
	}
	if _, err := hk.Generate(8161, nil); err == nil {
		t.Errorf("unexpected success")
	}
}

func TestPRK_MustGenerate_PanicsForTooLongKeyRequest(t *testing.T) {
	defer func() {
		if r := recover(); r == nil {
			t.Errorf("unexpected success")
		}
