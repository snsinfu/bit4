    ...
    struct spatial_hash
    {
        ...
        void for_each_neighbor(F func)
        {
            ...
            for (std::size_t center = 0; center < n_buckets; ++center) {
                auto const& center_members = buckets_[center].members;

                for (std::size_t delta : hash_deltas_) {
                    std::size_t const other = (center + delta) % n_buckets;
                    auto const& other_members = buckets_[other].members;
      ...
      private:
        ...
        void init_hash_deltas()
        {
            ...
            std::size_t const n_buckets = buckets_.size();
            std::size_t const coord_deltas[] = {
                n_buckets - 1, n_buckets, n_buckets + 1
            };
    ...
    struct point_cloud
    {
      ...
      private:
        void update()
        {
            ...
            hash_.for_each_neighbor([&](std::size_t i, std::size_t j) {
