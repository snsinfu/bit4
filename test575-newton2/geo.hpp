// geo - minimal point geometry constructs for C++11
...
namespace geo
{
    ...
    namespace detail
    {
        // identity is the identity metafunction. This is used to get around ambiguous template type
        // deduction errors.
        template<typename T>
        struct identity
        {
    ...
    // basic_coords is a static container of N coordinate values. This is the base class of
    // geo::vector and geo::point.
    ...
    public:
        ...
        // iterator is the type of an iterator used to iterate over coordinate values.
        using iterator = scalar_type*;

        // const_iterator is the type of an iterator used to iterate over coordinate values with
        // only read access.
        ...
        // begin returns a random access iterator pointing to the first coordinate value.
        ...
        // end returns a random access iterator pointing to past the last coordinate value.
        ...
        // Indexing operator returns a reference to the i-th coordinate value. Behavior is undefined
    ...
    // Equality comparison `u == v` compares the coordinate values for *exact* floating-point
    // equality.
    template<typename T, index N>
    ...
    // Inequality comparison `u != v` compares the coordinate values for *exact* floating-point
    // inequality.
    template<typename T, index N>
    bool operator!=(basic_coords<T, N> const& u, basic_coords<T, N> const& v) noexcept
    {
    ...
    public:
        ...
        // Division broadcasts to all components. Behavior is undefined if a is zero.
        ...
        // squared_norm returns the squared Euclidean norm of this vector. This function is faster
        // to compute than norm.
        ...
        // cross returns the cross product of this vector and v. Compilation fails if this function
        // is called on a non-three-dimensional vector.
        ...
        // normalize returns the unit vector that is parallel to this vector. Behavior is undefined
    ...
    // Addition `u + v` returns a vector whose coordinates are the element-wise sum of u and v.
    template<typename T, index N>
    ...
    // Subtraction `u - v` returns a vector whose coordinates are the element-wise difference of u
    // and v.
    template<typename T, index N>
    vector<T, N> operator-(vector<T, N> const& u, vector<T, N> const& v) noexcept
    {
    ...
    // Division by scalar `v / a` returns a vector v scaled by 1/a. Behavior is undefined if a is
    // zero.
    template<typename T, index N>
    ...
    // dot returns the dot product of a pair of vectors. This is the same as the member function
    // `u.dot(v)`.
    template<typename T, index N>
    ...
    // squared_norm returns the squared Euclidean norm of a vector. This is the same as the member
    // function `v.squared_norm()`.
    template<typename T, index N>
    ...
    // norm returns the Euclidean norm of a vector. This is the same as the member function
    // `v.norm()`.
    template<typename T, index N>
    ...
    // cross returns the cross product of a pair of three-dimensional vectors. This is the same as
    // the member function `u.cross(v)`.
    template<typename T>
    ...
    // normalize returns the unit vector parallel to v. Behavior is undefined if v is a zero vector.
    // This is the same as the member function `v.normalize()`.
    template<typename T, index N>
    ...
    public:
        ...
        // squared_distance returns the squared Euclidean distance to p. This function is faster to
    ...
    // Subtraction `p - q ` returns the displacement vector originates at q and pointing to p.
    template<typename T, index N>
    ...
    // squared_distance returns the squared Euclidean distance between two points. This function is
    // faster to compute than distance.
    template<typename T, index N>
    ...
    // centroid returns the centroid of points in given range. Behavior is undefined if the range
    // is empty.
